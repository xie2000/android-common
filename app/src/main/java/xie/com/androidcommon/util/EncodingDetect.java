...
public class EncodingDetect {
	public static final int TYPE_ASSETS_FILE = 1;// assets文件
	public static final int TYPE_SDCARD_FILE = 2;// sdcard文件
	 ...
	 *            :文件path
	 * @param type
	 *            :类型(assets文件或sdcard文件)
	 * @return
	 */
	public static String getFileEncode(String filePath, int type) {
...
class BytesEncodingDetect extends Encoding {
	...
	public int detectEncoding(String path, int type) {
		InputStream chinesefile;
		byte[] rawtext = null;
		try {
			if (type == EncodingDetect.TYPE_ASSETS_FILE) {
				chinesefile = MyApplication.getInstance().getAssets()
						.open(path);
			} else {
				chinesefile = new FileInputStream(path);
			}

			if (chinesefile.available() <= 1024) {
				rawtext = new byte[chinesefile.available()];
			} else {
				rawtext = new byte[1024];
			}
